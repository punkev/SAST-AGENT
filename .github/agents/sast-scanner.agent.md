...
2. Discover repository structure, source roots, build manifests, framework indicators, and frontend/backend boundaries. Perform a rapid **Semantic Pre-Triage Scan** for dangerous patterns (raw SQL concat, exec, eval, XML parsers, deserialization sinks, etc.) and prioritize queuing endpoints associated with these files. Record results in `repo-profile.md` and `technology-detection.md`.
